...
// Localization
function secure_invites_local_init(){
	// error_log('Secure invites !!!!'.dirname(plugin_basename(__FILE__)).'/');
	$domain = 'secure_invite';
  //$locale = apply_filters('plugin_locale', get_locale(), $domain);
    // load_textdomain($domain, WP_LANG_DIR.'/booked/'.$domain.'-'.$locale.'.mo');

  load_plugin_textdomain($domain, false ,dirname( plugin_basename( __FILE__ ) ) . '/languages/');
}
// secure_invites_local_init();
add_action('plugins_loaded', 'secure_invites_local_init');
